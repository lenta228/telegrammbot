document.addEventListener('DOMContentLoaded', () => {
    ...
    // Анимация появления секции социальных сетей при прокрутке
    ...
    if (socialSection) {
        ...
        const socialObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting && scrollingDown && !isVisible) {
                    // Показываем секцию только при прокрутке вниз
                    isVisible = true;
                    requestAnimationFrame(() => {
                    ...
                    // Скрываем секцию при прокрутке вверх
                    isVisible = false;
                    requestAnimationFrame(() => {
        ...
        const handleScroll = throttle(() => {
            ...
            if (isInView && scrollingDown && !isVisible) {
                // Показываем при прокрутке вниз
                isVisible = true;
                requestAnimationFrame(() => {
                    socialSection.classList.add('visible');
                });
            ...
            } else if ((!isInView || !scrollingDown) && isVisible) {
                // Скрываем при прокрутке вверх или выходе из области видимости
                isVisible = false;
                requestAnimationFrame(() => {
                    socialSection.classList.remove('visible');
                    socialCards.forEach(card => {
    ...
    // Добавляем эффект свечения для карточек социальных сетей
    const socialCardsElements = document.querySelectorAll('.social-card');
    const handleSocialCardMouseMove = throttle((e, card) => {
        const rect = card.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const y = e.clientY - rect.top;
        
        requestAnimationFrame(() => {
            const glowSize = '120px';
            card.style.background = `radial-gradient(${glowSize} at ${x}px ${y}px, rgba(255, 255, 255, 0.08), transparent), linear-gradient(145deg, #111111, #000000)`;
        });
    }, 30);

    socialCardsElements.forEach(card => {
        card.addEventListener('mousemove', (e) => handleSocialCardMouseMove(e, card));
        card.addEventListener('mouseleave', () => {
            requestAnimationFrame(() => {
                card.style.background = 'linear-gradient(145deg, #111111, #000000)';
            });
    ...
    // Добавляем звуковой эффект при клике на карточки (опционально)
    socialCardsElements.forEach(card => {
